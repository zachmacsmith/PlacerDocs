...
- `factor_breakdown` (`FactorBreakdown`) — the five-factor decomposition `P(accept) = 𝟙[gates pass] × p_fit_residual × p_want × p_cap × p_resp`, with `gates_pass` (boolean indicator that all hard gates cleared), `blocking_gate_ids` (list of any gate IDs that blocked), `want_provenance` (tracing whether the want signal came from an edge posterior, self-consistency, evidence, or prior), and a `cap_interval`.
...
The formal contract for the Value operator is declared in `placer/core/contracts.py` as `ValueContract`, which specifies that `value(candidate, order_state, beliefs) → Valuation` must be a **pure function** — no side effects, no mutations. `factor_breakdown` is contractually mandatory in every output.

## Implementation — how it works

Value is deliberately **type-only** at the `placer/value/` level. `placer/value/types.py` contains the output schema (`Valuation`, `ContactCost`) and imports `FactorBreakdown` from `placer.core.events`. No computation lives in this module; implementations must satisfy `ValueContract` (defined in `placer/core/contracts.py`).

**Data flow through the wider system:**

1. `FactorBreakdown` (defined in `placer.core.events`) is the shared type that carries `gates_pass`, `p_fit_residual`, `p_want`, `p_cap`, `p_resp`, `want_provenance`, `cap_interval`, and `blocking_gate_ids`. Its `p_accept` property evaluates `𝟙[gates_pass] × p_fit_residual × p_want × p_cap × p_resp`. It is imported by Value types, the contracts module, the candidates store, and the projections layer — making it the authoritative decomposition object across the pipeline.

2. Once a `Valuation` is computed, its fields are mirrored into two downstream frozen records:
   - `ValuationSnapshot` (`placer/identity/candidates.py`) — frozen onto each `Candidate` record at decision time; includes `sd` and `exploration_slot`.
   - `ValuationSnapshotPayload` (`placer/core/events.py`) — written to the event spine as a `decision.valuation_snapshot` event, providing a bitemporal audit trail.
...
The `ValueContract` abstract base class in `placer/core/contracts.py` declares the pure-function boundary explicitly: any implementation must accept `(Candidate, OrderWorldState, BeliefStore)` and return a `Valuation` synchronously, with no async I/O permitted by the signature.
